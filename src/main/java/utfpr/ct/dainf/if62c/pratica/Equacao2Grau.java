...
public class Equacao2Grau<T extends Number> {
    ...
    public double getRaiz1(){
        if( (Math.pow(b.doubleValue(),2) - 4*a.doubleValue()*c.doubleValue()) < 0.0 )
            throw new RuntimeException("Equação não tem solução real");
        
        return ((-1*b.doubleValue() + 
                Math.sqrt( Math.pow(b.doubleValue(),2) - 4*a.doubleValue()*c.doubleValue()))/2*a.doubleValue() );
